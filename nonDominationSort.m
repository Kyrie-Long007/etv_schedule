function result = nonDominationSort(chrom, M, V)
% 对初始种群开始排序 快速非支配排序
% 使用非支配排序对种群进行排序。该函数返回每个个体对应的排序值和拥挤距离，是一个两列的矩阵。  
% 并将排序值和拥挤距离添加到染色体矩阵中 
% x(i,V + k) < x(j,V + k)说明i支配j

[N, ~] = size(chrom);% N为矩阵x的行数，也是种群的数量
front = 1;
F(front).f = [];
individual = [];
...
for i = 1 : N
    ...
    for j = 1 : N
        ...
        for k = 1 : M        %判断个体i和个体j的支配关系
            if (chrom(i,V + k) < chrom(j,V + k))  
                dom_less = dom_less + 1;
            elseif (chrom(i,V + k) == chrom(j,V + k))
    ...
    if individual(i).n == 0 %个体i非支配等级排序最高，属于当前最优解集，相应的染色体中携带代表排序数的信息
        chrom(i,M + V + 1) = 1; %在V+M后面再开一列用于存放等级
        	...
        	for j = 1 : length(individual(F(front).f(i)).p)%循环个体i所支配解集中的个体
            	individual(individual(F(front).f(i)).p(j)).n = ...%...表示的是与下一行代码是相连的， 这里表示个体j的被支配个数减1
        	   	 ...
        	   	 if individual(individual(F(front).f(i)).p(j)).n == 0% 如果q是非支配解集，则放入集合Q中
               		chrom(individual(F(front).f(i)).p(j),M + V + 1) = ...%个体染色体中加入分级信息
                        front + 1;
...
[~,index_of_fronts] = sort(chrom(:,M + V + 1));%对个体的代表排序等级的列向量进行升序排序 index_of_fronts表示排序后的值对应原来的索引
for i = 1 : length(index_of_fronts)
    sorted_based_on_front(i,:) = chrom(index_of_fronts(i),:);%sorted_based_on_front中存放的是x矩阵按照排序等级升序排序后的矩阵
end
current_index = 0;
...
result = z();%得到的是已经包含等级和拥挤度的种群矩阵 并且已经按等级排序排序